...
export const userResolvers = {
  ...
  Mutation: {
    ...
    forgotPassword: async (
      parent: any,
      { input }: { input: { email: string; _id: string } },
      context: any,
    ) => {
    ...
    resetPassword: async (
      ...
      {
        input,
      }: { input: { oldPassword: string; newPassword: string; token: string } },
      context: any,
    ) => {
    ...
    updateUser: async (
      parent: any,
      {
        ...
        input: { username?: string; phoneNo?: string; role?: "USER" | "ADMIN" };
      },
      context: any,
    ) => {
      const userContext = requireAuth(context?.user);
      ...
      if (updateData?.phoneNo) {
        if (existingUser?.phoneNo === updateData?.phoneNo) {
          throw new GraphQLError(
            "Phone number is same, Please provide a different phone number",
            {
      ...
      Object.keys(updateData).forEach((field: string) => {
        if (
    ...
    accountDeactivationRequest: async (
      parent: any,
      ...
      if (!user.isActive)
        throw new GraphQLError("Account is already deactivated", {
          extensions: { code: "BAD_REQUEST" },
        });
      ...
      const htmlContent = callAccountDeactivationInHtml(
        user.username,
        deactivationDate,
      );
      await sendVerificationEmail(
    ...
    cancelDeactivationRequest: async (
      parent: any,
      { input }: { input: { _id: string } },
      context: any,
    ) => {
      ...
      if (!user.scheduledDeactivationAt)
        throw new GraphQLError("No pending account deactivation found", {
          extensions: { code: "BAD_REQUEST" },
        });
